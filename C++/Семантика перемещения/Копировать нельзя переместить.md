...
Вернёмся к задаче о считалке Иосифа. В заготовке кода была структура, которая подсчитывает количество копирований, и вам пришлось решать задачу, используя перемещение. Разберёмся, как эта структура была устроена на примере другой похожей структуры:
...
Начнём с простого. Специальное слово `default` говорит, что компилятор должен сам решить, как будет происходить перемещение. Часто это удобно. Компилятор просто применит перемещение к каждому члену класса или структуры. Если для члена класса определён конструктор перемещения, компилятор будет стараться его найти. Если хотя бы один из членов класса переместить нельзя, компилятор не справится с поставленной задачей и сообщит об ошибке. Вот что случится, если удалить перемещающий конструктор и оператор присваивания:
...
struct NoncopyableInt {
    // пусть членом структуры станет неперемещаемый тип
    NonmovingInt value;
...
📖 rvalue-ссылки в С++, как и другие типы ссылок — отдельная большая тема, которая не покрывается этим курсом. В качестве дополнительного чтения можно посоветовать, например, статьи: [первую](https://habr.com/ru/post/348198/) и [вторую](https://habr.com/ru/post/322132/).
